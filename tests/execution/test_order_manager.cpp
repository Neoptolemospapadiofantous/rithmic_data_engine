...
// ─────────────────────────────────────────────────────────────────────────────
// Ghost-fill recovery tests (today's incident: stale stop from prior session)
// ─────────────────────────────────────────────────────────────────────────────

// 22. Unknown fill while FLAT sets entry_halted_, blocking new signals.
TEST(ghost_fill_halts_entries) {
    ...
    ASSERT(f.om.is_flat());          // OM still tracks FLAT internally
    ASSERT(f.om.is_entry_halted());  // but entries are blocked

    // Signal should be rejected
    size_t before = f.sent_baskets.size();
    f.om.on_signal(OrbSignal::BUY, 19100.0, "orb_breakout");
    ASSERT_EQ(f.sent_baskets.size(), before);  // no new order sent
    ASSERT(f.om.is_flat());
}

// 23. confirm_exchange_flat() clears the ghost-fill halt, re-enabling entries.
//     Simulates the tid=451 snapshot arriving after manual RTrader close.
TEST(confirm_exchange_flat_clears_ghost_halt) {
    ...
    // Entries should be unblocked
    f.om.on_signal(OrbSignal::BUY, 19100.0, "orb_breakout");
    ASSERT_EQ(f.om.state(), PosState::PENDING_ENTRY);
}

// 24. A second unknown fill while already halted is treated as MANUAL-CLOSE-DETECTED
//     (operator closing via RTrader) — it must NOT escalate or send anything.
TEST(second_fill_while_halted_is_manual_not_ghost) {
    Fixture f;

    // First ghost fill — expected
    ...
    // Second unknown fill (manual RTrader close) — should not send anything
    f.om.on_fill_notification("RTRADER-CLOSE", 19010.0, 2, /*is_entry=*/false);

    ASSERT_EQ(f.cancelled_baskets.size(), cancels_before);  // no extra cancel
    ASSERT_EQ(f.sent_baskets.size(), sends_before);          // no extra order
...
//     not GHOST-FILL, and does not add to entry_halted_.
...
// 26. register_unwind_basket(): when halted by a ghost fill, a subsequent
//     registered-unwind fill clears entry_halted_ automatically.
TEST(register_unwind_basket_clears_ghost_halt) {
    Fixture f;

    // Ghost fill from a stale stop (e.g. cancel raced with trigger)
    ...
    ASSERT(!f.om.is_entry_halted());  // halt cleared by tracked unwind
    ASSERT(f.om.is_flat());
}

// 27. Full stale-stop recovery flow:
//     stale stop fires → halt → register unwind → unwind fills → halt cleared →
//     entries re-enabled.
TEST(stale_stop_full_recovery_flow) {
    Fixture f;

    // Step 1: prior-session stop fires while we think we're flat
    f.om.on_fill_notification("OLD-SESSION-STOP", 19000.0, 1, /*is_entry=*/false);
    ASSERT(f.om.is_entry_halted());
    ASSERT(f.om.is_flat());

    // Step 2: signal rejected while halted
    size_t sends = f.sent_baskets.size();
    f.om.on_signal(OrbSignal::BUY, 19100.0, "orb_breakout");
    ASSERT_EQ(f.sent_baskets.size(), sends);  // blocked

    // Step 3: startup-recon sends market unwind and registers it
    f.om.register_unwind_basket("RECON-UNWIND-XYZ");

    // Step 4: user also closes via RTrader — second unknown fill while halted
    ...
    ASSERT(!f.om.is_entry_halted());  // halt cleared

    // Step 6: new entries now allowed
...
int main() {
    ...
    RUN(second_fill_while_halted_is_manual_not_ghost);
